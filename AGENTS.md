...
| # | 템플릿 | 언제 | 핵심 제약 |
|---|---|---|---|
| 1 | `01-github-handle` 🪪 | **체크인 때** | 제목 `[handle] <핸들>`. `@` 없이. **Copilot 라이선스가 하나도 안 붙은 개인 계정**이어야 한다 → `hale-righthon`. QR(`images/ghcp-check.png`)로 화면 확인 |
| 2 | `02-team-building` 🏷️ | **결과 제출 전 아무 때나** | 제목 `[team] <팀이름>`. **마감은 없지만 제출 전 필수.** 팀 이름 **고유**(대소문자 무시, 선점되면 다른 이름). **리더 = 이슈 작성자**여야 함. 단독 참가면 멤버 칸 비워도 됨. 같은 리더로 재제출 = **업데이트**(중복 생성 안 됨) → 이름 바꾸려면 그냥 다시 내면 된다 |
| 3 | `03-result-submission` 🚀 | **~16:30** | 앱 제목 · **공개 레포 URL** · **커밋 해시** · **배포 URL** · (선택)설명 |
